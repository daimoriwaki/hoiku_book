...
#ggsave("~/dev/hoiku_book/hoiku_book_working/ch1/figure/1_5.png", width = 8, height = 6, dpi = 300)
...
ggplot(filtered_data, aes(x = 職種, y = 推定年収, fill = 都道府県)) +
  geom_bar(
    ...
    position = "dodge",
    width = 0.8,
    color = "white",   # 境界線を白にする（棒グラフの間に微小な隙間を作る）
    size = 1.0       # 境界線の太さ
  ) +
  scale_fill_manual(values = c("gray0", "gray20", "gray40", "gray65", "gray85")) +  # グレースケール
  geom_text(
    aes(label = paste0(都道府県, "\n", round(推定年収))),  
  ...
  scale_x_discrete(limits = rev(levels(filtered_data$職種))) +  # ★X軸の並び順を逆にする
  labs(
    y = "推定年収 (万円)",
    x = ""
  ) +
